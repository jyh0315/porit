...
function computeClusterExposure() {
  const clusterType = CONFIG.clusteringResidueByEnv[environment];
  const particles = allVisibleParticles().filter(p => p.type === clusterType);
  const result = [];
...
function stepSolvent(dtFactor) {
  ...
  const exposedList = computeClusterExposure().filter(e => e.exposure > 0);
...
function renderSolvent() {
  ...
  for (const w of solventParticles) {
    if (w.caged) {
      // 정렬된(카고) 물: 진하고 또렷하게 — "붙잡혀 있는" 느낌
      fill(74, 127, 214, 150);
      circle(w.x, w.y, CONFIG.solventRadius * 2.4);
    } else {
      // 자유로운(벌크) 물: 옅고 흐리게, 더 빠르게 스쳐 지나가는 느낌
      fill(154, 200, 224, 85);
      circle(w.x, w.y, CONFIG.solventRadius * 1.6);
    }
  }
}
